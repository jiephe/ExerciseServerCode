	...
	: m_port(port), m_ThreadNum(nThreadNum)
{
	...
	m_ListenThreadPool.Init(m_ThreadNum);
...
// 			m_ListenThreadPool.AddTask(pTask);
		...
		{
			CDispatchTask* pTask = new CDispatchTask(e_WriteType, write_set.fd_array[i], this);
			m_ListenThreadPool.AddTask(pTask);
		}

		for (u_int i = 0; i < excep_set.fd_count; i++)
		{
			CDispatchTask* pTask = new CDispatchTask(e_ExcepType, excep_set.fd_array[i], this);
			m_ListenThreadPool.AddTask(pTask);
		}
	}
}
